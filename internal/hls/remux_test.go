...
// conversion.
func TestFFmpegFoundWithoutPath(t *testing.T) {
	if FFmpegPath() == "" {
	...
	ffmpegMu.Lock()
	ffmpegPath = "" // drop the cached hit so the lookup really reruns
	ffmpegMu.Unlock()
...
func TestFFmpegHonoursDMFFmpegEnv(t *testing.T) {
	real := FFmpegPath()
	if real == "" {
		t.Skip("ffmpeg is not installed")
	}
