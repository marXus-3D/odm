...
// Command dm-installer is the single setup program for DM. It carries the
...
func main() {
	uninstall := flag.Bool("uninstall", false, "remove DM instead of installing it")
	...
	noStartup := flag.Bool("no-startup", false, "do not start DM when signing in")
	...
	if err := RunWizard(*uninstall); err != nil {
		fmt.Fprintln(os.Stderr, "dm-installer:", err)
		os.Exit(1)
	}
}
